...
namespace Gluvi {
	...
	//=================================================================================
	enum MOUSE { NOBODY, CAMERA, WIDGETS, USER };
	static MOUSE mouse_owner = NOBODY;
...
#define FILENAMELENGTH 256
	...
	void draw_3d_arrow(const float base[3], const float point[3], float arrow_head_length)
	{
		...
		Vector3F w(point[0] - base[0], point[1] - base[1], point[2] - base[2]);
		double len = mag(w);
		...
		Vector3F u = cross(w, Vector3F(0, 1, 0));

		// If vector is parallel to the y-axis, use the x-axis
		if (mag(u) == 0)
			u = cross(w, Vector3F(1, 0, 0));

		// get a vector perp to w and u
		Vector3F v = cross(w, u / mag(u));
		v = v / mag(v);
	...
	void draw_2d_arrow(const Vector2F base, const Vector2F point, float arrow_head_length)
	{
		//glPushAttrib(GL_CURRENT_BIT|GL_ENABLE_BIT|GL_LINE_BIT);
		//glDisable(GL_LIGHTING);

		Vector2F w = point - base;
		double len = mag(w);
		...
		//	                     -1  0
		Vector2F u = Vector2F(1 * w[1], -1 * w[0]);
		u = u / mag(u);

		// v = w - 90 (in fact v=-u)
		Vector2F v = Vector2F(-1 * w[1], 1 * w[0]);
		v = v / mag(v);
